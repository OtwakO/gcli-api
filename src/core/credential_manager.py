...
class CredentialManager:
    ...
    def load_credentials(self):
        ...
        if self._load_from_env():
            logger.info(
                f"Loaded {len(self._credentials)} credentials from CREDENTIALS_JSON_LIST env var."
            )
        else:
            self._load_from_files()
            if self._credentials:
                logger.info(
                    f"Loaded {len(self._credentials)} credentials from oauth_creds_*.json files."
                )
    ...
    def _add_credential_from_info(self, cred_info: dict, source: str = "env"):
        if "refresh_token" not in cred_info:
            logger.warning(
                f"Skipping a credential from {source} due to missing 'refresh_token'."
            )
            return
    ...
    async def get_next_credential(self) -> Optional[ManagedCredential]:
        ...
        async with self._lock:
            ...
            for _ in range(len(self._credentials)):
                ...
                self._next_credential_index = (self._next_credential_index + 1) % len(
                    self._credentials
                )
